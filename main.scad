...
module assemble()
{
    ...
    for(side = [0:3]) {
        for (i = [1:vertical_mounts]) {
            translate([x_offsets[side], y_offsets[side], (i*(box_height/(vertical_mounts+1))) - (bracket_width/2)]) rotate([0, -90, angles[side]]) bracket_with_screws(panel_thickness);
        }
    }

    // horizontal connectors, front and back
    for (i = [1:horizontal_mounts]) {
        translate([(i*(total_width()/(horizontal_mounts+1))) - (bracket_width/2), panel_thickness, box_height]) rotate([270, 0, 0]) bracket_with_screws(panel_thickness);
    }
    for (i = [1:horizontal_mounts]) {
        translate([(i*(total_width()/(horizontal_mounts+1))) + (bracket_width/2), box_depth+panel_thickness, box_height]) rotate([270, 0, 180]) bracket_with_screws(panel_thickness);
    }

    // horizontal connectors, left and right
    for (i = [1:horizontal_mounts]) {
        translate([panel_thickness, (i*(total_depth()/(horizontal_mounts+1))) + (bracket_width/2), box_height]) rotate([270, 0, -90]) bracket_with_screws(panel_thickness);
    }
    for (i = [1:horizontal_mounts]) {
        translate([box_width+panel_thickness, (i*(total_depth()/(horizontal_mounts+1))) - (bracket_width/2), box_height]) rotate([270, 0, 90]) bracket_with_screws(panel_thickness);
    }
